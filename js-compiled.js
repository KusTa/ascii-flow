...
m = [">", "<", "^", "v"],
ba = k.concat(m),
n = "ontouchstart" in window || "onmsgesturechange" in window;
function p(a, b) {
    this.x = a;
    this.y = b;
}
...
function ja(a) {
    ...
    var c = r(F(a, new p(0, 0)), new p(3, 3)),
    d = F(a, new p(a.canvas.width, a.canvas.height)).add(new p(3, 3));
    ...
    for (var e = c.x; e < d.x; e++) {
        b.moveTo(9 * e - a.offset.x, 0 - a.offset.y),
        b.lineTo(9 * e - a.offset.x, 17 * a.state.cells.length - a.offset.y);
    }
    for (e = c.y; e < d.y; e++) {
        b.moveTo(0 - a.offset.x, 17 * e - a.offset.y),
        b.lineTo(9 * a.state.cells.length - a.offset.x, 17 * e - a.offset.y);
    }
    a.context.stroke();
    e = !a.B;
    ...
    for (var g = c.x; g < d.x; g++) {
        for (var l = c.y; l < d.y; l++) {
            ...
            if (B(h) || null != h.h && " " != A(h)) {
                a.context.fillStyle = null != h.h ? "#DEF": "#F5F5F5",
                b.fillRect(9 * g - a.offset.x, 17 * (l - 1) - a.offset.y, 9, 17);
            }
    ...
    if (a.B) {
        ...
        for (e = c.x; e < d.x; e++) {
            for (h = !1, g = c.y; g < d.y; g++) {
                l = G(a.state, new p(e, g)),
                B(l) && g != d.y - 1 || !h || (b.moveTo(9 * e - a.offset.x + 4.5, 17 * h - a.offset.y - 8.5), b.lineTo(9 * e - a.offset.x + 4.5, 17 * (g - 1) - a.offset.y - 8.5), h = !1),
                B(l) && !h && (h = g);
            }
        }
        for (g = c.y; g < d.y; g++) {
            for (h = !1, e = c.x; e < d.x; e++) {
                l = G(a.state, new p(e, g)),
                B(l) && e != d.x - 1 || !h || (b.moveTo(9 * h - a.offset.x + 4.5, 17 * g - a.offset.y - 8.5), b.lineTo(9 * (e - 1) - a.offset.x + 4.5, 17 * g - a.offset.y - 8.5), h = !1),
                B(l) && !h && (h = e);
            }
        }
        a.context.stroke();
...
function J(a, b, c, d, e) {
    ...
    for (d = d ? b.y: c.y; l++<s;) {
        var u = new p(l, d),
        H = a.getContext(new p(l, d));
        " " == e && 2 == H.j + H.i || K(a, u, e);
    }
    for (; h++<g;) {
        u = new p(D, h),
        H = a.getContext(new p(D, h)),
        " " == e && 2 == H.left + H.right || K(a, u, e);
    }
    L(a, b, e);
    L(a, c, e);
    K(a, new p(D, d), e);
...
f.move = function(a) {
    N(this.state);
    var b = this.state.getContext(this.a),
    c = this.state.getContext(a);
...
function ka(a, b) {
    this.state = a;
    this.value = b;
    n && ($("#freeform-tool-input").val(""), $("#freeform-tool-input").hide(0,
    function() {
        $("#freeform-tool-input").show(0,
        function() {
...
f.move = function() {};
f.end = function() {
    null != this.a && (this.b = this.a, this.a = null, $("#text-tool-widget").hide(0,
    function() {
...
f.g = function() {
    ...
    d = 0; d < a.length; d++) {
        if ("\n" == a[d]) { (c++, b = 0)
            ...
            if (reg.test(a[d])) { (L(this.state, this.b.add(new p(b, c)), a[d]), b = b + 2);
            } else { (L(this.state, this.b.add(new p(b, c)), a[d]), b++);
            }
        }
    }
};
                ...
                if (d = c[s], e = 0 != z[h].x, g = -1 != m.indexOf(A(G(this.state, a))), l = -1 != m.indexOf(A(G(this.state, d))), 1 == E(this.state.getContext(d))) {
                    ...
                    for (var D in z) {
                        if (0 != z[h].add(z[D]).length() && 2 != z[h].add(z[D]).length()) {
                            var u = na(this, d, z[D]);
                            0 != u.length && (u = u[0], b.push({
                                position: u,
                                o: e,
                                T: g,
                                W: l,
                                S: -1 != m.indexOf(A(G(this.state, u)))
                            }));
                        }
                    }
                }
...
f.move = function(a) {
    ...
    for (b in this.e) {
        this.e[b].T && L(this.state, a, "^"),
        this.e[b].S && L(this.state, this.e[b].position, "^"),
        this.e[b].W && L(this.state, new p(this.e[b].o ? this.e[b].position.x: a.x, this.e[b].o ? a.y: this.e[b].position.y), "^");
    }
};
f.end = function() {
    O(this.state);
...
function ua(a, b, c) {
    ...
    for (e = 0; e < b.length; e++) {
        for (var g = b[e], l = 0; l < g.length; l++) {
            var h = g.charAt(l); - 1 != k.indexOf(h) && (h = "+"); - 1 != m.indexOf(h) && (h = "^");
...
T.prototype.r = function() {
    ...
    $(window).keydown(function(a) {
        ...
        if (a.ctrlKey || a.metaKey) {
            67 == a.keyCode && (c = "<copy>"),
            86 == a.keyCode && (c = "<paste>"),
            90 == a.keyCode && sa(this.state),
            89 == a.keyCode && ta(this.state),
            88 == a.keyCode && (c = "<cut>");
        }
...
function va(a, b) {
    ...
    $("#drive-filename").click(function() {
        var a = "" + $("#drive-filename").text(),
        a = prompt("Enter new filename:", a);
        this.file.title = a;
        this.save();
        za(this);
    }.bind(this));
...
function W(a, b) {
    window.gapi.auth.authorize({
        client_id: "125643747010-9s9n1ne2fnnuh5v967licfkt83r4vba5.apps.googleusercontent.com",
        scope: "https://www.googleapis.com/auth/drive",
        immediate: b
    },
    function(a) { ! a || a.error || this.t || (this.t = !0, $("#drive-button").addClass("active"), window.setTimeout(function() {
            Ba(this);
        }.bind(this), 500));
    }.bind(a));
}
...
function Ba(a) {
    1 < window.location.hash.length && ($("#drive-save-state").text("Loading..."), X(window.gapi.client.request({
        path: "/drive/v2/files/" + window.location.hash.substr(1, window.location.hash.length - 1),
        method: "GET"
    }),
    function(a) {
        Ca(this, a);
...
function Da(a, b) {
    var c = "\r\n---------314159265358979323846\r\nContent-Type: application/json\r\n\r\n" + JSON.stringify({
        title: null == a.file ? "Untitled ASCII Diagram": a.file.title,
        mimeType: "text/plain"
    }) + "\r\n---------314159265358979323846\r\nContent-Type: text/plain\r\n\r\n" + b + "\r\n---------314159265358979323846--";
    return window.gapi.client.request({
        path: "/upload/drive/v2/files" + (null == a.file ? "": "/" + a.file.id),
        method: null == a.file ? "POST": "PUT",
        params: {
...
function Fa(a, b) {
    var c = window.gapi.auth.getToken().access_token,
